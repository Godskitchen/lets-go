...
export async function cardController(fastify: FastifyInstance) {
  ...
  fastify.get<{Querystring: GetCardsQuery, Params: {cardId: string}}>(
    '/:cardId',
    {
      schema: {
        tags: ['cards'],
        description: 'Получение списка карточек пользователей с учетом тех предпочтений, которые были указаны в форме. Возможна фильтрация по странам, а также по континентам. Также предусмотрена пагинация - по умолчанию отдается 4 первых карточки, включая основную карточку пользователя.',
